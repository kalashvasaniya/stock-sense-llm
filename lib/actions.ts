...
export async function fetchStockData(query: string): Promise<StockData> {
  try {
    ...
    const response = await fetch("http://13.127.23.138:8000/run-graph", {
      method: "POST",
      headers: {
    ...
    if (!data || !data.graph_data) {
      ...
      throw new Error(
        "Invalid response format from server: missing graph_data"
      );
    }
    ...
    try {
      ...
      const cleanedGraphData = data.graph_data
        .replace(/\n/g, "")
        .replace(/\s+/g, " ");
    ...
    if (!graphData.price_data || !graphData.volume_data) {
      ...
      throw new Error(
        "Missing required data in graph_data: price_data or volume_data is missing"
      );
    }
